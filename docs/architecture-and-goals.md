...
- **AI tiers**: (1) rules/statistics + local document extraction (pdfplumber/Tesseract) —
  always on, no LLM; (2) ~~local LLM (Ollama)~~ **deferred, not built** (decided 2026-07-29);
  (3) **Gemini free tier** (the single cloud provider — text + vision) — per-user **opt-in
  only**, off by default
...
> **AI decision (2026-07-29):** the **local-LLM tier (Ollama) is dropped as a priority and
> not built.** In practice most intelligence is Tier 1 (deterministic rules/statistics — all
> insights, forecast, dedup, category rules, and the *local* document extraction via
> pdfplumber/Tesseract, which needs no LLM). The **only AI is Gemini** (free tier, opt-in,
> off by default). So the tiers below describe the *intended* allocation; where a row says
> "Local AI," today that means **rule-based local extraction**, and the LLM part happens on
> **Gemini only when the user opts in** — otherwise it degrades to the deterministic local
> result, never silently to cloud. Ollama can be revisited later if "nothing ever leaves the
> machine, even for hard cases" becomes a hard requirement. DeepSeek/other paid or non-US
> providers are rejected (cost + financial-data residency). See `plan.md` Principle 3.

| Tier | What | Where data goes | Default |
|---|---|---|---|
| 1 | Rules + statistics + local extraction (parsing, insights, forecast, pdfplumber/Tesseract) | nowhere | always on |
| 2 | ~~Ollama local LLM~~ — **deferred, not built** (2026-07-29) | — | — |
| 3 | Gemini free tier (sole cloud provider; NVIDIA/Groq removed in T5) | Google | **off**; per-user opt-in with warning |

### AI task allocation — what runs where (decided; "Local AI" = rules today, Ollama deferred)
