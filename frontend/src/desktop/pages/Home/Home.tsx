...
import { formatCurrency, formatDate, formatSignedAmount } from '../../../shared/utils/format.ts'
...
export const Home = (): JSX.Element => {
  ...
  return (
    <Box>
      {/* Y3: above the grid on purpose — the point is that you see it without looking
          for it. Renders nothing when there's nothing wrong, so the grid moves up. */}
      <NeedsAttention />
      {/* Z2: bento grid. Tiles of deliberately unequal weight, so the eye lands on the
          one number that matters before anything else — rather than two even columns
          where everything competes. Content and logic below are unchanged from the
          column layout; only the container and each tile's span are new. */}
      <Box className={styles.bento}>
        {/* Safe-to-Spend Hero — I5, wired to GET /api/insights/safe-to-spend */}
        <Card className={`${styles.balanceCard} ${styles.tileHero}`}>
        ...
        {/* This Month sits beside the hero — the two numbers people check together. */}
        <Card className={`${styles.contentCard} ${styles.tileMonth}`}>
        ...
        {/* Cash on hand / Credit owed — never summed, a credit balance is debt, not
            spendable money */}
        <Flex gap="3" className={styles.tileCash}>
        ...
        {/* Accounts */}
        <Box className={styles.tileAccounts}>
          <Flex className={shared.sectionHeader}>
        ...
        <Box className={styles.tileInsights}>
          <InsightCards />
        </Box>

        {/* Recent Activity */}
        <Card className={`${styles.contentCard} ${styles.tileActivity}`}>
          <Flex className={shared.sectionHeader}>
          ...
          {recentTransactions.length === 0 ? (
            ...
            <Box className={styles.txList}>
              {recentTransactions.map((t) => (
                <Flex key={t.id} className={styles.txRow} align="center" justify="between" gap="3">
        ...
        {/* Upcoming Bills */}
        <Card className={`${styles.contentCard} ${styles.tileBills}`}>
          <Flex className={shared.sectionHeader}>
